    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (checkFirstname && checkLastname) {
            try {
                if (customer_type == 0) {
                    String sql = "SELECT"
                               + " DATE_FORMAT(pr.visit_date, '%m/%d/%Y'), p.id, u.first_name, u.middle_name, u.last_name, p.days_per_week, p.cardio_time, p.exercise_time_goal, p.daily_weight"
                    ...
                    if (rs.next()) {
                        ...
                        request.getRequestDispatcher("/searchexerciseResult.jsp").forward(request, response);
                    } else {
